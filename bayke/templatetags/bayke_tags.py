...
from bayke.models.sites import BaykePermission
...
@register.inclusion_tag("baykeshop/banners.html")
...
def navbar_result(context):
    ...
    return {
        ...
        'navs': BaykeProductCategory.get_cates()[:bayke_settings.HOME_NAV_COUNT],
        'search_value': search_value,
        'search_action':f'{reverse("bayke:product-list")}',
        'categorys': categorys
    } 
    
    
@register.inclusion_tag("baykeshop/head_top.html")
def head_top(request):
    return {
        'cartsnum': BaykeCart.get_cart_count(request.user) if request.user.is_authenticated else 0,
        'request': request
    }
